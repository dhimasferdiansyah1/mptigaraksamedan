...
import { DeliveryNoteSchema, FakturSchema } from "@/lib/main/validationsMain";
...
import { Faktur, PurchaseOrder } from "@prisma/client";
...
import { createFakturDetail } from "@/app/(dashboard)/dashboard/main/detail/[id]/faktur/[fakturId]/action";

export default function TambahFaktur({ fakturId }: { fakturId: Faktur }) {
  const form = useForm<z.infer<typeof FakturSchema>>({
  ...
  const handleSubmitFaktur = async (values: z.infer<typeof FakturSchema>) => {
  ...
  return (
    <Form {...form}>
      {fakturId ? (
        <form
          onSubmit={form.handleSubmit(handleSubmitFaktur)}
                      ...
                      <FormControl>
                        <UploadDropzone
                          ...
                          onClientUploadComplete={(res) => {
                            ...
                            form.setValue("foto1_fk", res[0].url);
                          }}
                      ...
                      <FormControl>
                        <UploadDropzone
                          ...
                          onClientUploadComplete={(res) => {
                            ...
                            form.setValue("foto2_fk", res[0].url);
                          }}
                          onUploadError={(error: Error) => {
                            alert(`ERROR! ${error.message}`);
                          }}
